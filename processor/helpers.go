...
func (p *Processor) checkBlockExists(db *pgxpool.Pool) (bool, error) {
	var count int
	err := db.QueryRow(context.Background(), `select count(*) from blocks where block_hash = $1`, p.Block.BlockHash).Scan(&count)
	if err != nil {
...
func (p *Processor) checkBlockReorged(db *pgxpool.Pool) (bool, error) {
	var count int
	err := db.QueryRow(context.Background(), `select count(*) from blocks where number = $1`, p.Block.Number).Scan(&count)
	if err != nil {

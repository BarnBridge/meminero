...
create or replace function smart_alpha.junior_tokens_not_redeemed(addr text, pool text, ts bigint) returns numeric(78, 18)
    language plpgsql as
$$
declare
...
create or replace function smart_alpha.senior_tokens_not_redeemed(addr text, pool text, ts bigint) returns numeric(78, 18)
    language plpgsql as
$$
declare
    balance_not_redeemed numeric;
...
create or replace function smart_alpha.junior_tokens_redeemed_at_active_epoch(addr text, pool text, ts text) returns numeric(78, 18)
    language plpgsql as
$$
begin
    return coalesce(( select sum(a.tokens_in)
                     ...
                     where a.user_address = addr
                       ...
                       and a.epoch_id = (select smart_alpha.pool_active_epoch_at_ts(pool, ts))
                       and a.tranche = 'JUNIOR' ), 0 );
end;
$$;


create or replace function smart_alpha.senior_tokens_redeemed_at_active_epoch(addr text, pool text, ts text) returns numeric(78, 18)
    language plpgsql as
$$
begin
    return coalesce(( select sum(a.tokens_in)
...
declare
    ...
    junior_redeemed_at_active_epoch numeric;
begin
    ...
    select into junior_redeemed_at_active_epoch * from smart_alpha.junior_tokens_redeemed_at_active_epoch(addr, pool, ts) jtr;

    select into junior_balance balance + junior_not_redeemed + junior_redeemed_at_active_epoch
...
declare
    ...
    senior_redeemed_at_active_epoch numeric;
begin
    ...
    select into senior_redeemed_at_active_epoch * from smart_alpha.senior_tokens_redeemed_at_active_epoch(addr, pool, ts) str;

    select into senior_balance balance + senior_not_redeemed + senior_redeemed_at_active_epoch

create function yield_farming.yf_sum_actions_by_token(a_type staking_action_type, _token_address text,
...
$$
begin
    return query select coalesce(d.point, w.point),
                        coalesce(d.amount, 0) as sum_deposits,
                        coalesce(w.amount, 0) as sum_withdrawals
                 from yield_farming.yf_sum_actions_by_token('DEPOSIT', _token_address, _start_time, _end_time, scale) d
                          full outer join yield_farming.yf_sum_actions_by_token('WITHDRAW', _token_address, _start_time,
                                                                                _end_time, scale) w
                                          on d.point = w.point;
end
$$;

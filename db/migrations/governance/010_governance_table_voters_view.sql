...
select user_address,barn.balance_of(user_address) as bond_staked,
       ...
       ( select count(*) from governance.governance_abrogation_votes where lower(user_id) = lower(barn_users.user_address) ) as votes,
       ( select count(*) from governance.governance_proposals where lower(proposer) = lower(barn_users.user_address) )         as proposals,

create function governance.proposal_votes(id bigint)
    returns TABLE
            (
                user_id         text,
                support         boolean,
...
$$
begin
    return query select distinct v.user_id,
                                 first_value(v.support)
                                 over (partition by v.user_id order by v.block_timestamp desc) as support,
                 ...
                 where proposal_id = id
                   and ( select count(*)
                         from governance.votes_canceled vc

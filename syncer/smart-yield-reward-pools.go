...
type SmartYieldRewardPool struct {
...
type SmartYieldRewardPools []SmartYieldRewardPool

func (p SmartYieldRewardPools) Sync(tx pgx.Tx) error {
	if len(p) == 0 {
		return nil
	}

	start := time.Now()
	log.WithField("count", len(p)).Info("syncing smart yield reward pools")
	defer func() {
		log.WithField("duration", time.Since(start)).Info("done syncing smart yield reward pools")
	}()
	...
	for _, pool := range p {
		...
		if err != nil {
			return errors.Wrap(err, "could not insert smart yield reward pool")
		}
	}
